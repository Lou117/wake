...
class ResponseFactory
{
    ...
    public static function createRedirectResponse(string $location): ResponseInterface
    {
        ...
        return (new Response(302))
            ->withHeader(self::HTTP_HEADER_LOCATION, $location);
    }
     ...
     * @return void
     */
    public static function sendToClient(ResponseInterface $response): void
    {
        if (headers_sent() === false) {
            foreach ($response->getHeaders() as $name => $values) {
                ...
                foreach ($values as $value) {
                    header(sprintf(
                        ...
                        $name,
                        $value
                    ), $replace, $response->getStatusCode());
                }
            }
            ...
            if (
                ...
                header(
                    self::HTTP_HEADER_CONTENT_LENGTH.': '.$response->getBody()->getSize(),
                    true,
                    $response->getStatusCode()
                );
            }
